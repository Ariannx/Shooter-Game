...
def angle_direction(point1, point2):
    x1, y1 = point1
    x2, y2 = point2
...
def uploadLevel(level: str):
    with open(f'gameLevels/level{level}_data.csv', newline='') as csvfile:
...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while run:
        ...
        clock.tick(FPS)
        ret, frame_ia = cap.read()

        frame_game = cv2.cvtColor(frame_ia, cv2.COLOR_BGR2RGB)
        frame_game = cv2.rotate(frame_game, cv2.ROTATE_90_COUNTERCLOCKWISE)

        # Recolor image to RGB
        image = cv2.cvtColor(cv2.flip(frame_ia, 2), cv2.COLOR_BGR2RGB)
        ...
        try:
            ...
            # Calculate angle
            granada = angle_direction(right_shoulder, right_wrist)
            dispara = angle_direction(left_shoulder, left_wrist)
            salta = angle_direction(right_knee, left_knee)
            walk = angle_direction(left_shoulder, nose)
            ...
            pass
        surf = pygame.surfarray.make_surface(frame_game)
